package com.srk.srklocationservices.ui.locationapis.reversegeocoding
...
class SRKGoogleReverseGeoCodingAPI(srkLocationBuilder: SRKLocationBuilder) :
    ...
    private fun prepareGeoCodingURL(): String {
        ...
        if (location.first) {
            builder.append("&").append(PARAM_LAT_LNG).append(location.second)
        } else {
                        ...
                        onLocationListener.onLocationDetailsFetched(try {
                            if (response.isSuccessful && response.body() != null) {
                                val placeDetailsResponse =
                                    response.body() as PlaceDetailsResponse
